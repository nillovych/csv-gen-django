...
document.getElementById('add-form-btn').addEventListener('click', handleAddForm);
document.getElementById('submit-btn').addEventListener('click', handleSubmit);

function handleDeleteForm(button) {
    const form = button.closest('form');
    form.remove();
    if (document.getElementById('forms-container').querySelectorAll('form').length < maxForms) {
...
function handleAddForm() {
    ...
    if (formCount <= maxForms) {
        ...
        if (formCount >= maxForms) {
            document.getElementById('add-form-btn').style.display = 'none';
        }
    }
}
...
function handleSubmit(event) {
    ...
    const orders = {};
    const inputOrders = document.querySelectorAll('input[name="order"]');
    let allValid = true;

    inputOrders.forEach(input => {
        const orderValue = input.value.trim();
...
function submitForm() {
    ...
    // Assuming main_id is dynamically inserted using Django template
    fullVal.main_id = "{{ main_rows.0.id }}";
    fullVal.name = name;
    fullVal.sep = sep;
    fullVal.stch = stch;
...
function removeIntegerFields(select) {
    const nextElement = select.parentNode.nextElementSibling;
    if (nextElement && nextElement.classList.contains("int_field")) {
        nextElement.remove();
        const secondNextElement = select.parentNode.nextElementSibling;
        if (secondNextElement && secondNextElement.classList.contains("int_field")) {
